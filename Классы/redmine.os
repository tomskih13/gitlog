...
Функция СформироватьПользовательскиеПоляRedmine(ДанныеЗадачи) Экспорт
	...
	Для Каждого Поле Из redmineCustomFields Цикл
		...
		Если ТипЗнч(value) = Тип("Массив") И value.Количество() = 0 Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ДобавитьПользовательскоеПолеRedmine(МассивПолей, value, id); //, name);
...
Процедура ДобавитьПользовательскоеПолеRedmine(МассивПолей, value, id) //, name)
	
	СтруктураПоля = Новый Структура("value, id", value, id); //, name);
...
Функция СформироватьДанныеЗадачи(ДанныеЗадачи) Экспорт
	...
	ЗаголовокЗадачиKaiten = ДанныеЗадачи.Получить("title");
	Если ЗаголовокЗадачиKaiten <> Неопределено Тогда
		ПозицияНомераНавигатора = СтрНайти(ЗаголовокЗадачиKaiten, "]");
		Если ПозицияНомераНавигатора > 0 Тогда
			ЗаголовокЗадачиKaiten = СокрЛП(Сред(ЗаголовокЗадачиKaiten, ПозицияНомераНавигатора + 1, СтрДлина(ЗаголовокЗадачиKaiten)));
		КонецЕсли;
	Иначе
		ЗаголовокЗадачиKaiten = cardId; // Возможно будет пустое описание
	КонецЕсли;
	issue.Вставить("subject"			, ЗаголовокЗадачиKaiten);
...
Функция ПроверитьСформированныеДанныеЗадачиRedmine(ДанныеЗадачиRedmine) Экспорт
	...
	description = ДанныеЗадачиRedmine.issue.description;
...
Функция ОтправитьНомерЗадачиRedmineВKaiten(ИДЗадачиRedmine) Экспорт
	
	//https://name.kaiten.ru/api/latest/cards/{card_id}
	
	ШаблонЗапроса	= "https://%1.kaiten.ru/api/latest/cards/%2";
	urlЗапроса		= СтрШаблон(ШаблонЗапроса, kaitenNameSpace, cardId);
	
	idНомера		= kaitenUsersFields.Получить("НомерЗадачиRedmine");
	Если idНомера = Неопределено Тогда
		Сообщить("Не удалось определить id пользовательского поля kaiten ""Номер задачи Redmine""");
		Возврат Неопределено;
	КонецЕсли;
	
	properties = Новый Структура();
	properties.Вставить(idНомера, ИДЗадачиRedmine);
	
	ПараметрыЗапроса = Новый Структура("properties", properties);
	Ответ			 = ОтправитьЗапросВKaiten(urlЗапроса, "PATCH", ПараметрыЗапроса);
	
	Возврат Ответ;
	
КонецФункции

Функция ПроверитьРезультатОбновленияНомераЗадачиRedmineВKaiten(РезультатОбновления) Экспорт
	...
	Если properties = Неопределено Тогда
		РезультатПроверки.Результат = Ложь;
		РезультатПроверки.ТекстОшибки = НСтр("ru ='Не удалось получить пользовательские поля Kaiten'");
	Иначе
		idНомера = kaitenUsersFields.Получить("НомерЗадачиRedmine"); // Ранее проверялся
		НомерЗадачиRedmine = properties.Получить(idНомера);
		Если НомерЗадачиRedmine = Неопределено Тогда
			РезультатПроверки.Результат = Ложь;
			РезультатПроверки.ТекстОшибки = НСтр("ru ='Не удалось получить номер задачи Redmine'");
		КонецЕсли;
	КонецЕсли;
