...
Процедура СоздатьЗадачуРедмайн(ЗадачаКайтен)
	...
	Если Вложения <> Неопределено Тогда
		ДобавитьВЛог("Предварительная обработка вложений", ИДЗадачиКайтен);
...
Процедура ОбработатьВложенияЗадачиКайтен(ЗадачаКайтен, Вложения)
	...
	Для Каждого Вложение Из Вложения Цикл
		...
		Если Не ЗначениеЗаполнено(АдресВложения) Тогда
			Продолжить;
		КонецЕсли;
		
		Если СтрНайти(ФункциональныеТребования, АдресВложения) > 0 ИЛИ СтрНайти(ТребованияРазработки, АдресВложения) Тогда
	...
	Попытка
		...
		Если Ответ.КодСостояния = 201 Тогда
			...
			Если Токен <> Неопределено Тогда
				Результат = Новый Структура("token, filename, localUrl", Токен, ИмяФайла, localUrl);
...
Функция ЗаменитьСсылкиВложенийВОписанииЗадачиРедмайн(Описание, ДанныеЗадачи)
	...
	Для Каждого Загрузка Из Загрузки Цикл
		...
		localUrl = "(" + Загрузка.localUrl + ")";
		ПозицияСсылки = СтрНайти(Описание, localUrl);
		Если ПозицияСсылки = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ПозицияВосклицательногоЗнака = СтрНайти(Описание, "![", НаправлениеПоиска.СКонца, ПозицияСсылки, 1);
		Если ПозицияВосклицательногоЗнака = 0 Тогда
			Описание = СтрЗаменить(Описание, localUrl, "!" + Загрузка.filename + "!");
		Иначе
			Описание = Сред(Описание, 1, ПозицияВосклицательногоЗнака - 1)
				+ "!" + Загрузка.filename + "!"
				+ Сред(Описание, ПозицияСсылки + СтрДлина(localUrl));
		КонецЕсли;
