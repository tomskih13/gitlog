...
Перем СтруктураПользовательскихПолей; // Структура ID пользовательских полей
...
Функция Инициализировать(Знач АргументыКоманднойСтроки) Экспорт
	...
	kaitenDeveloperFieldId	= НастройкиИзФайла.Получить("kaitenDeveloperFieldId");
	kaitenAnaliticFieldId	= НастройкиИзФайла.Получить("kaitenAnaliticFieldId");
	
	СтруктураПользовательскихПолей = Новый Структура();
	СтруктураПользовательскихПолей.Вставить("Разработчик"	, kaitenDeveloperFieldId);
	СтруктураПользовательскихПолей.Вставить("Аналитик"		, kaitenAnaliticFieldId);
...
Процедура ДополнитьДанныеЗадачиKaiten(ДанныеЗадачи) Экспорт
	...
	Для Каждого КлючИЗначение Из СтруктураПользовательскихПолей Цикл
		...
		Для Каждого УИД Из МассивУИДПользователей Цикл
			...
			ИскомыеСтроки = usersTable.НайтиСтроки(Отбор);
