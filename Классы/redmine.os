...
Процедура СоздатьОбработатьЗадачиРедмайн() Экспорт
	...
	Если ЗначениеЗаполнено(ИДЗадачиКайтенВх) Тогда
		...
		СоздатьОбработатьЗадачиРедмайнПоКолонкеКайтен(0, ЗадачиКайтен, ДанныеДляОбработки);
...
Процедура СоздатьОбработатьЗадачиРедмайнПоКолонкеКайтен(ИДКолонкиКайтен, ЗадачиКайтен, ДанныеДляОбработки)
...
Процедура ОбработатьСозданнуюЗадачуРедмайн(НомерЗадачиРедмайн, ЗадачаКайтен, ДанныеДляОбработки, ИДКолонкиКайтен)
	...
	ИДОтветственныйАналитик = 0;
	ИДТестировщик = 0;
	ИДФА = 0;
	ЗаполнитьЗначенияИДПользовательскихПолейДляОбработкиУсловий(custom_fields, ИДОтветственныйАналитик, ИДТестировщик, ИДФА);
		...
		Для Каждого Условие Из МассивУсловий Цикл
			...
			Если analiticsAssigned = Истина Тогда
				УсловиеВыполняется = УсловиеВыполняется = Истина
					И (ИДНазначенаНа = ИДОтветственныйАналитик ИЛИ ИДНазначенаНа = ИДТестировщик);
			КонецЕсли;
			
			analiticsFAAssigned = Условие.Получить("analiticsFAAssigned");
			Если analiticsFAAssigned = Ложь Тогда
				УсловиеВыполняется = УсловиеВыполняется = Истина
					И (ИДНазначенаНа <> ИДОтветственныйАналитик
					И ИДНазначенаНа <> ИДТестировщик
...
Процедура ЗаполнитьЗначенияИДПользовательскихПолейДляОбработкиУсловий(custom_fields, ИДОтветственныйАналитик, ИДТестировщик, ИДФА)
	...
	Для Каждого Поле Из custom_fields Цикл
		...
		Если ЭтоПолеОтвАналитик = Истина Тогда
			ЗаполнитьЗначениеИДПользовательскогоПоля(Поле, ИДОтветственныйАналитик);
			Продолжить;
		КонецЕсли;
		
		ЭтоПолеТестировщик = Поле.Получить("id") <> Неопределено И Поле.Получить("id") = 50;
		Если ЭтоПолеТестировщик = Истина Тогда
			ЗаполнитьЗначениеИДПользовательскогоПоля(Поле, ИДТестировщик);
			Продолжить;
		КонецЕсли;
		
		ЭтоПолеФА = Поле.Получить("id") <> Неопределено И Поле.Получить("id") = 2;
		Если ЭтоПолеФА = Истина Тогда
			ЗаполнитьЗначениеИДПользовательскогоПоля(Поле, ИДФА);
		КонецЕсли;
		
	КонецЦикла;
	
	Если ИДОтветственныйАналитик = Неопределено Тогда
		ИДОтветственныйАналитик = 0;
	КонецЕсли;
	
	Если ИДТестировщик = Неопределено Тогда
		ИДТестировщик = 0;
	КонецЕсли;
	
	Если ИДФА = Неопределено Тогда
		ИДФА = 0;
	КонецЕсли;
